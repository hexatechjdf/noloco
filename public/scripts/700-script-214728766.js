(async () => {
            window.mainScript = document.querySelector('#creditReportScript');
            let contactData = '';
            let uid = mainScript.getAttribute('data-uid') ?? null
    ...
    `;
      ...
      `;
            ...
            function injectCreditButton(selector) {
                ...
                if (isValidLocation(uid, locationId)) {
            ...
            async function isValidLocation(uid, locationId) {
                try {
                    const res = await fetch('/api/credit-report/check/valid/location', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
